...
// -----------------------------------------------------------------------------
// Identify Optima in Metric Matrices
// -----------------------------------------------------------------------------
...
DataFrame pick_optima_cpp(List metric_list, NumericVector i_vals, int tau_max, int L_size, bool strict_monotonic = false, bool find_min = false) {
  ...
  int c = tau_max; // 0-based index representing lag 0
  ...
  for (int k = 0; k < n; ++k) {
    ...
    // Lambda to evaluate a local region of length L_size
    auto check_window = [&](int start_idx) -> bool {
      double extreme_val = find_min ? R_PosInf : R_NegInf;
      int extreme_idx = -1;

      // Find extreme value and index, ignoring NAs
